...
namespace BioBaseCLIA.Run
{
    public partial class frmWorkList : frmParent
    {
                            ...
                            if (DiluteName != "1")
                            {
                                ...
                                for (int i = 0; i < diuList.Count; i++)
                                {
                                    ...
                                    if (drRg.Length > 0)
                                    {
                                        ...
                                        {
                                            string diulest = OperateIniFile.ReadIniData("ReagentPos" + drRg[g]["Postion"].ToString(), "leftDiuVol", "", iniPathReagentTrayInfo);
        ...
        void CalculatResult(int testid, int pmt)
        {
            ...
            {               
                ...
                if (sampleType == "血清" || sampleType == "尿液" || sampleType == "体液" || sampleType == "定标液")
                {
                    ...
                    {
                        samplenametemp = XmlOperation.PredictStart + (int.Parse(samplenametemp.Substring(samplenametemp.Length - 3, 3)) % int.Parse(PredictNum));
                    }
                    else
                    {
                        samplenametemp = XmlOperation.PredictStart + int.Parse(samplenametemp.Substring(samplenametemp.Length - 3, 3));
                    }
                }               
            ...
            BeginInvoke(new Action(() =>
            {
                ...
                if ((testResult == null) || ((testResult.SampleType == null) || (testResult.SampleType.Length == 0)))//null/空直接跳出
                {
                    goto null_0xo1;
                }
        ...
        void Goon()
        {
            ...
            if (tss.Count > 1)
                _GaDoingOne = tss[tss.Count - 1];
            if (TestStep != null)//在进行实验调度前点暂停，防止第一个实验不加样 2020/8/13
                TestStep = GaNextOne();
            LoadingHelper.CloseForm();
            NetCom3.ComWait.Set();
            frmSampleLoad.CaculatingFlag = false;
