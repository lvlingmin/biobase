    ...
    public partial class frmScaling : frmParent
    {
        ...
        public void RefreshUI()
        {
            ...
            if (dtProject != null)
            {
                ...
                {
                    string ss = dtProject.Rows[i]["ShortName"].ToString();
                    db = new DbHelperOleDb(3);
                    DataTable dt = bllregent.GetList(" ReagentName = '" + dtProject.Rows[i]["ShortName"].ToString() + "' and Status='正常'").Tables[0];
                    if (dt != null)
                    {
                        for (int j = 0; j < dt.Rows.Count; j++)
                        {
                            if (dt.Rows[j]["Batch"].ToString() == "") continue;
                            db = new DbHelperOleDb(1);
                            bool ExitsMainCurve = bllMainCurve.ExistsCurve(dtProject.Rows[i]["ShortName"].ToString(), dt.Rows[j]["Batch"].ToString());
                            DataRow[] ScalCurve = dtScalResult.Select("ItemName = '" + dtProject.Rows[i]["ShortName"].ToString()
                                + "' and RegentBatch = '" + dt.Rows[j]["Batch"].ToString() + "' and Status = 1");
                            string ScalType = "";
                            string ActiveDate = "";
                            if (ScalCurve.Length == 0) continue;
                            int ExpiryDate = int.Parse(dtProject.Rows[i]["ExpiryDate"].ToString());
                            ...
                            dtScalInfo.Rows.Add(dtProject.Rows[i]["ShortName"].ToString(), dt.Rows[j]["Batch"].ToString(),
                                ExitsMainCurve ? "Y" : "N", ScalCurve.Length > 0 ? "Y" : "N", ScalType,
                                ActiveDate, (Convert.ToDateTime(ActiveDate).AddDays(ExpiryDate)).ToString("yyyy-MM-dd"), Convert.ToInt32(dtProject.Rows[i]["ExpiryDate"]));//2018-11-14 zlx add
                            //dt.Rows[j]["AddDate"]
                        }
                    }
                    else
                    {
                        dtScalInfo.Rows.Add(dtProject.Rows[i]["ShortName"].ToString(), "",
                                   "N", "N", "",
                                  "", "", Convert.ToInt32(dtProject.Rows[i]["ExpiryDate"]));//2018-07-30 zlx add
                    }
                }
            }
        ...
        void ShowScalCurve(string itemname, string batch)
        {
            ...
            dc = new drawCurve();            
            //数据库项目表中查询定标方程选择字段
            int CalMode = int.Parse(DbHelperOleDb.GetSingle(@"select CalMode from tbProject where ShortName = '"+ itemname + "'").ToString());
