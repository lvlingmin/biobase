    ...
    public partial class frmResultQuery : frmParent
    {
        ...
        private void fbtnQuery_Click(object sender, EventArgs e)
        {
            ...
            //dgvSampleData.DataSource = null;
             ...
             DataTable dtPatientInfo = DbHelperOleDb.Query(@str).Tables[0];
...
//                                                         + dtpEndDate.Value.AddDays(1).ToString("yyyy-MM-dd") + "#").Tables[0];
        ...
        private void fbtnRecalculate_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                for (int i = 0; i < dgvSampleData.SelectedRows.Count; i++)
                {
                    ...
                    DataTable tbtbProject = DbHelperOleDb.Query(@"select RangeType,ValueRange1,MinValue,MaxValue from tbProject where ShortName = '" + dgvSampleData.SelectedRows[i].Cells["ItemName"].Value + "'").Tables[0];
                    ...
                    if (concentration != "非数字" && dgvSampleData.SelectedRows[i].Cells["ItemName"].Value != "")
        ...
        private string GetResult(string ProName, double concentration)
        {
            string Result = "";
            DbHelperOleDb db = new DbHelperOleDb(0);
            DataTable tbtbProject = DbHelperOleDb.Query(@"select RangeType,ValueRange1,MinValue,MaxValue from tbProject where ShortName = '" + ProName + "'").Tables[0];
        ...
        private void fbtnSaveData_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dgvSampleData.SelectedRows.Count; i++)
            {
                DbHelperOleDb.ExecuteSql(@"update tbAssayResult set Concentration = " + Convert.ToDouble(
        ...
        private void fbtnPrint_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            else if (fbtnPrint.Text == "确定")
            {
                ...
                object SampleType = DbHelperOleDb.GetSingle(@"select SampleType from tbSampleInfo where SampleID = "
                ...
                object Department = DbHelperOleDb.GetSingle(@"select Department from tbSampleInfo where SampleID = "
                                                         + dgvSampleData.Rows[0].Cells["SampleID"].Value.ToString());
                if (Department == null)
                {
                    Department = " ";
                ...
                object SendDoctor = DbHelperOleDb.GetSingle(@"select SendDoctor from tbSampleInfo where SampleID = "
                                                         + dgvSampleData.Rows[0].Cells["SampleID"].Value.ToString());
                if (SendDoctor == null)
                {
                    SendDoctor = " ";
                ...
                object SendDateTime = DbHelperOleDb.GetSingle(@"select SendDateTime from tbSampleInfo where SampleID = "
                                                       + dgvSampleData.Rows[0].Cells["SampleID"].Value.ToString());
                ...
                object InspectDoctor = DbHelperOleDb.GetSingle(@"select InspectDoctor from tbSampleInfo where SampleID = "
                                                       + dgvSampleData.Rows[0].Cells["SampleID"].Value.ToString());
                ...
                //2018-11-12 zlx add
                object CheckDoctor = DbHelperOleDb.GetSingle(@"select CheckDoctor from tbSampleInfo where SampleID = "
                                              + dgvSampleData.Rows[0].Cells["SampleID"].Value.ToString());
                if (CheckDoctor == null)
                {
                    CheckDoctor = "";
...
//            #endregion
        ...
        private void dgvPatientInfo_SelectionChanged(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow dgv in dgvSampleData.Rows)
            {
                ...
                DbHelperOleDb db = new DbHelperOleDb(0);
                DataTable tbtbProject = DbHelperOleDb.Query(@"select RangeType,ValueRange1,MinValue,MaxValue from tbProject where ShortName = '" + dgv.Cells["ItemName"].Value + "'").Tables[0];
                //2018-11-26 zlx mod
        ...
        private void fbtnTestAgain_Click(object sender, EventArgs e)
        {
            ...
            if (_dtSP.Rows.Count > 0 )
            {
                if (Convert.ToInt32(_dtSP.Rows[0]["Status"]) == 1)
                {
                    for(int i=0;i<dtSpInfo.Rows.Count;i++)
                    {
                        ...
                        {
                            if (DbHelperOleDb.ExecuteSql(@"update tbSampleInfo set Status = 0  where SampleID=" + dgvSampleData.SelectedRows[0].Cells["SampleID"].Value + "") > 0)
                            { 
            ...
            int bUpdate=0;
            if(_boolAgain)
                bUpdate = DbHelperOleDb.ExecuteSql(@"update tbAssayResult set Status = -1  where SampleID=" + dgvSampleData.SelectedRows[0].Cells["SampleID"].Value + " AND ItemName='" + dgvSampleData.SelectedRows[0].Cells["ItemName"].Value + "'AND PMTCounter=" + dgvSampleData.SelectedRows[0].Cells["PMTCounter"].Value + "");
            if (bUpdate > 0)
