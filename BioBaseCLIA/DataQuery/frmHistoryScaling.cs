...
namespace BioBaseCLIA.DataQuery
{
    public partial class frmHistoryScaling : frmSmallParent
    {
        ...
        private void frmHistoryScaling_Load(object sender, EventArgs e)
        {
            ...
            //2018-08-25 zlx mod
            DataTable dtScalingResult = DbHelperOleDb.Query(@"select ScalingResultID,ItemName,ActiveDate,iif(Status=1, '是', '') AS bstatus from tbScalingResult 
        ...
        private void fbtnChoice_Click(object sender, EventArgs e)
        {
            ...
            {
                DbHelperOleDb db = new DbHelperOleDb(1);
                DbHelperOleDb.ExecuteSql(@"update tbScalingResult set Status=1 where ItemName = '" + dgvExistScal.CurrentRow.Cells["ItemName"].Value + "'AND ScalingResultID = "
                                                       + Convert.ToInt32(dgvExistScal.CurrentRow.Cells["ScalingResultID"].Value) + "").ToString();
                DbHelperOleDb.ExecuteSql(@"update tbScalingResult set Status=0 where ItemName = '" + dgvExistScal.CurrentRow.Cells["ItemName"].Value + "'AND ScalingResultID <> "
