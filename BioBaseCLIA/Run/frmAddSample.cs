...
namespace BioBaseCLIA.Run
{
    public partial class frmAddSample : frmSmallParent
    {
        ...
        /// 样本装载最大数量
        /// </summary>
        const int SampleNum = 50;//2019-02-25  zlx add
        /// <summary>
        /// 最大样本转载量
        /// </summary>
        const int SampleLoadMax = 10;//2019-04-09 zlx add
        public frmAddSample()
        {
            InitializeComponent();
        ...
        private void frmAddSample_Load(object sender, EventArgs e)
        {
            ...
            if (dtSampleInfo.Rows.Count >= SampleNum && dtSampleInfo.Select("Status=0").Length >= SampleNum)//y add 20180425 2018-09-07 zlx add dtSampleInfo.Select("Status=0").Length>=60
            {//y add 20180425
        ...
        private string AutoNumber()//2018-11-28 zlx mod 
        {
            ...
            DbHelperOleDb db = new DbHelperOleDb(1);
            object emergency = DbHelperOleDb.GetSingle("select max(SampleNo) from tbSampleInfo where left(SampleNo,8)='" + autoNumber + "' AND Right(SampleNo,3)<'999'");
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            else if (((Button)sender).Text == "开始扫码")
            {
                ...
                if (int.Parse(txtScanStartNo.Text.Trim()) > 50 || int.Parse(txtScanEndNo.Text.Trim()) > SampleNum)
                {
                    frmMsg.MessageShow("添加样本", "请输入1-" + SampleNum + "的扫码位置！");
                    txtScanStartNo.Focus();
                    btnAdd.Enabled = true;
                    return;
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (((Button)sender).Text == "删除")
            {
                ...
                if (dgvSampleList.SelectedRows.Count > 0)
                {
                    ...
                    //    bUpdate = DbHelperOleDb.ExecuteSql(@"delete from tbSampleInfo where Status =0 AND SampleNo='" + dgvSampleList.SelectedRows[0].Cells[1].Value + "'");
                    //else
                    //    bUpdate = DbHelperOleDb.ExecuteSql(@"update tbSampleInfo set Status = 2  where SampleNo='" + dgvSampleList.SelectedRows[0].Cells[1].Value + "'");
                    //if (bUpdate > 0)
                    ...
                    //    }
                    //    #endregion
                    //    i = Convert.ToInt32(dgvSampleList.SelectedRows[0].Cells["Position"].Value);//y add 20180425

                    //    var drr = dtSampleInfo.Select("SampleNo='" + temp + "'");//y add 20180426
                    //    dtSampleInfo.Rows.Remove(drr[0]);//y modify 20180426

                    //    //add y 20180516
                    //    if (AutoUploadAndUnload1.Checked == true) SampleUploadOrUnload(i, 1, false);
                    //Jun mod  
                ...
                if (dtSampleInfo.Rows.Count < SampleNum)//y add 20180425
                {//y add 20180425
            ...
            {
                //正在进行样本实验，不允许再添加标准品 jun add 20190328
                ...
                if (addOrModify == 0)
                {
                    ...
                    */
                            ...
                            for (int j = 0; j < dtItemInfo.Rows.Count; j++)
                            {
                                if (ch.Text == dtItemInfo.Rows[j]["ShortName"].ToString())
                                {
                                    ...
                                    for (int ii = 0; ii < PointNum; ii++)
                                    {
                                        ...
                                        if (postion == SampleNum.ToString()) postion = "1";
                    ...
                    if (modelSp.CheckDoctor == null || modelSp.CheckDoctor == "")
                        modelSp.CheckDoctor = "";
                    //2018-11-15 zlx add
                    if (cmbSpType.Text.Contains("标准品"))
                    {
                ...
                dvv.Sort = "SampleNo";//Jun mod 20190314
            ...
            if (dgvSampleList.SelectedRows.Count > 0)
            {
                ...
                if (frmWorkList.EmergencyFlag || frmWorkList.addOrdinaryFlag)
                {
                    ...
                    for (int i = 0; i < dgvSampleList.SelectedRows.Count; i++)
                    {
                        ...
                        string emergency = DbHelperOleDb.GetSingle("select Emergency from tbSampleInfo where SampleNo = '"
        ...
        public void SelectInfo(int po = 0)//y modify 20180425原先的代码没有考虑datatable中的datarow不一定等于表格中首行的问题，改为选中行触发selectedrowschange事件
        {
            ...
            if (dgvSampleList.Rows.Count > 0 && po < SampleNum && po > 0)
            {
                int x = 0, y = 0;
                foreach (DataGridViewRow a in dgvSampleList.Rows)
                {
        ...
        private void btnMoreSave_Click(object sender, EventArgs e)
        {
            ...
            if (int.Parse(txtSpNum.Text) + int.Parse(txtSpStartPos.Text) - 1 > SampleNum)//this block y add 20180426
            {
                var drPos = dtSampleInfo.Select("Position='" + "1" + "' and Status=0");
                if (drPos.Length <= 0)
                    frmMsg.MessageShow("样本装载", "在当前选择的试剂盘位置加载此数量的样本可能会超出试剂盘的容量。如果想跨越" + SampleNum + "位和1位进行批量加载，请分开进行。");
                else
                    frmMsg.MessageShow("样本装载", "在当前选择的试剂盘位置加载此数量的样本会超出试剂盘的容量。");
                txtSpNum.Text = Convert.ToString(SampleNum - int.Parse(txtSpStartPos.Text) + 1);
                return;
            }
            /*
            //2018-11-27 zlx add
            ...
            for (int i = 0; i < int.Parse(txtSpNum.Text.Trim()); i++)
            {
                ...
                foreach (CheckBox ch in flpItemName.Controls)
                {
                    if (ch.Checked)
                    {
                        ...
                        if (PredictSet == "1")
                        {
                            XmlOperation.InsertSample(ch.Text, txtSpCode1.Text.Trim() + (int.Parse(txtSpCode2.Text) + i).ToString("000"));
                        }
                    }
                }
                if (PredictSet == "1")
        ...
        private int GetPos()
        {
            ...
            //return p + 1;
            //Jun mod 20190314            
            //Jun add 50位置没有东西认为是第一次加载，当50装载之后，从最低向上找
            //int p = 0;
            //if (!(dtSampleInfo.Select("Position='" + 50 + "' and Status = 0").Length > 0))
            //{
            //    for (int i = 0; i < dtSampleInfo.Rows.Count; i++)
            //    {
            //        if (p < int.Parse(dtSampleInfo.Rows[i]["Position"].ToString()) && int.Parse(dtSampleInfo.Rows[i]["Status"].ToString()) < 1)
        ...
        private string GetSpCodePart2()
        {
            ...
            else
            {
                //2018-11-28 zlx add
                DbHelperOleDb db = new DbHelperOleDb(1);
                object emergency = DbHelperOleDb.GetSingle("select max(SampleNo) from tbSampleInfo where left(SampleNo,8)='" + DateTime.Now.ToString("yyyMMdd") + "' AND Right(SampleNo,3)<'999'");
                //DataTable  emergency = DbHelperOleDb.Query("select max(SampleNo) from tbSampleInfo where SampleNo like '" + autoNumber + "???'").Tables();
                if (emergency!=null)
            ...
            {//判定单个录入时是否存在问题
                ...
                else if (dtSampleInfo.Select("Position='" + pos.ToString() + "' and Status < 2").Length > 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            DbHelperOleDb.ExecuteSql(@"update tbSampleInfo set Status = 2  where SampleNo='" + drPos1[i]["SampleNo"].ToString() + "'");
                ...
                for (int i = 0; i < num; i++)
                {
                    ...
                    else if (dtSampleInfo.Select("Position='" + (pos + i).ToString() + "' and Status < 2").Length > 0)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                DbHelperOleDb.ExecuteSql(@"update tbSampleInfo set Status = 2  where SampleNo='" + drPos1[k]["SampleNo"].ToString() + "'");
            ...
            else
            {
                ...
                for (int n = 0; n < dgvSampleList.SelectedRows.Count; n++)
                {
                    ...
                    if (Convert.ToInt32(dgvSelectedID[n]) == 0)
                        bUpdate = DbHelperOleDb.ExecuteSql(@"delete from tbSampleInfo where Status =0 AND SampleNo='" + dgvSampleList.SelectedRows[n].Cells["SampleNo"].Value + "'");
                    else
                        bUpdate = DbHelperOleDb.ExecuteSql(@"update tbSampleInfo set Status = 2  where SampleNo='" + dgvSampleList.SelectedRows[n].Cells["SampleNo"].Value + "'");
                    if (bUpdate > 0)
        ...
        private void txtSpPosition_Validated(object sender, EventArgs e)
        {
            ...
            if (content.Trim() != "")
            {
                ...
                if (((TextBox)sender) != txtSpNum && Convert.ToInt32(content) > SampleNum)
                {
                    frmMsg.MessageShow("样本装载", "输入的样本位置不存在!");//y modify 20180426
                    ((TextBox)sender).Text = "";
                    return;
                }
                if (((TextBox)sender) == txtSpNum && txtSpStartPos.Text != "" && (Int32.Parse(content) - 1 + Int32.Parse(txtSpStartPos.Text)) > SampleNum)//y modify 20180426
                {
        ...
        private bool SampleUploadOrUnload(int star, int length = 1, bool isUpload = true)
        {
            ...
            if (star < 1 || star > SampleNum || length + star > SampleNum+1 || length < 1)
            {
                MessageBox.Show("frmAddSample类中的方法SampleUpLoad(int star, int length)接收了不可接受的值，已退出该方法并返回false");
                return false;
            }
            ...
            else if (length > 2 && length <= SampleLoadMax) temp = " " + star + "号到" + (star + length - 1) + "号 ";
            else if (length > 2 && length > SampleLoadMax) temp = " " + star + "号到" + (star + SampleLoadMax-1) + "号 ";
            frmMsg.MessageShow("样本装载", "\t请在样本盘" + temp + "位置" + isup + "样本。\n\n点击“确定”或退出此对话框视为" + isup + "成功并开始进行下一步。");//2018-11-15 zlx mod
            if (!isUpload)
            {
                int loopMax = length;
                if (length > SampleLoadMax)
                {
                    loopMax = SampleLoadMax;
                }
                ...
                for (int n = 0; n < loopMax; n++)
                {
                    ...
                    if (Convert.ToInt32(dr[0]["Status"]) == 0)//[0]["Status"]
                        bUpdate = DbHelperOleDb.ExecuteSql(@"delete from tbSampleInfo where Status =0 AND SampleNo='" + dr[0]["SampleNo"] + "'");
                    else
                        bUpdate = DbHelperOleDb.ExecuteSql(@"update tbSampleInfo set Status = 2  where SampleNo='" + dr[0]["SampleNo"] + "'");
                    //2018-08-11 zlx mod
                    if (bUpdate > 0)
                    //if (bllsp.Delete(dgvSelectedID[n]))
                    {
                        ...
                        //var drRun = frmParent.dtSampleRunInfo.Select("SampleNo='" + dgvSelectedID[n - nullloop] + "'");
                        foreach (DataRow d in drRun)
                        {
                            frmParent.dtSampleRunInfo.Rows.Remove(d);
                        }
            ...
            if (length > SampleLoadMax)
            {
                return SampleUploadOrUnload(star + SampleLoadMax, length - SampleLoadMax, isUpload);
            }
            else
            {
                return true;
        ...
        private void fbLoadAll_Click(object sender, EventArgs e)//this function add y 20180516
        {
            ...
            bool isupload = false;
            SampleUploadOrUnload(1, SampleNum, isupload);
        ...
        private void fbLoadRun_Click(object sender, EventArgs e)//this function add y 20180516
        {
            ...
            if (star + length > 61)
            {
                frmMsg.MessageShow(temp + "样本", "样本" + temp + "的最大范围为从1号位到" + SampleNum + "号位。请重新检查输入，以确保“开始孔位”和“数量”之和不超过61.");
                fbLoadAll.Enabled = true;
                fbLoadRun.Enabled = true;
                return;
            }
            if (star < 1 || length < 1)
            {
                frmMsg.MessageShow(temp + "样本", "样本" + temp + "的最大范围为从1号位到" + SampleNum + "号位。请重新检查输入，以确保“开始孔位”和“数量”都不小于1.");
                fbLoadAll.Enabled = true;
                fbLoadRun.Enabled = true;
                return;
            }
