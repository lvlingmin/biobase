...
namespace BioBaseCLIA.Run
{
    public partial class frmTestResult : frmParent
    {
        ...
        private void fbtnSaveResult_Click(object sender, EventArgs e)
        {
            ...
            //2018-08-28 zlx mod
            DataTable UsingItem = DbHelperOleDb.Query(@"select ReagentName,Batch from tbReagent where Status = '正常'").Tables[0];
            ...
            for (int j = 0; j < UsingItemNew.Rows.Count; j++)
            {
               ...
               db1 = new DbHelperOleDb(0); 
                ...
                //该项目的历史定标
                int sameScaling = int.Parse(DbHelperOleDb.GetSingle(@"select count(*) from tbScalingResult where ItemName = '"
                                                           + lisCutoffResult[0].ItemName + "'").ToString());
                //将已有定标的状态设为非正在使用的状态
                if (sameScaling > 0)
                {
                    DbHelperOleDb.ExecuteSql(@"update tbScalingResult set Status=0 where ItemName = '"
                                                           + lisCutoffResult[0].ItemName + "'").ToString();
                }
        ...
        public void SaveStandardResult(List<TestResult> listResult)
        {
            ...
            for (int j = 0; j < UsingItemNew.Rows.Count; j++)
            {
                ...
                string calpointsConc = DbHelperOleDb.GetSingle(@"select CalPointConc from tbProject where ShortName = '"
                ...
                */
                ...
                else
                {
                    ...
                    object ScalingModel = DbHelperOleDb.GetSingle(@"select CalMethod from tbProject where ShortName = '"
        ...
        private void fbtnExportData_Click(object sender, EventArgs e)
        {
            ...
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    row.CreateCell(i).SetCellValue(dgvResultData.Columns[i + 1].HeaderText);
                ...
                for (int i = 0; i < dgvResultData.Rows.Count; i++)
                {
                    ...
                    for (int j = 1; j < dgvResultData.Columns.Count; j++)
                    {
                        //解决导出excel左上角三角形问题
                        try 
                        {
        ...
        private void fbtnTestAgain_Click(object sender, EventArgs e)
        {
            ...
            if (_dtSP.Rows.Count > 0)
            {
                if (Convert.ToInt32(_dtSP.Rows[0]["Status"]) == 1)
                {
                    for (int i = 0; i < dtSpInfo.Rows.Count; i++)
                    {
                        ...
                        {
                            if (DbHelperOleDb.ExecuteSql(@"update tbSampleInfo set Status = 0  where SampleID=" + dgvResultData.SelectedRows[0].Cells["SampleID"].Value + "") > 0)
                            {
            ...
            int bUpdate = 0;
                if(_boolAgain)
                    bUpdate = DbHelperOleDb.ExecuteSql(@"update tbAssayResult set Status = -1  where SampleID=" + dgvResultData.SelectedRows[0].Cells["SampleID"].Value + " AND ItemName='" + dgvResultData.SelectedRows[0].Cells["ItemName"].Value + "'AND PMTCounter=" + dgvResultData.SelectedRows[0].Cells["PMT"].Value + "");
            if (bUpdate > 0)
        ...
        private void functionButton1_Click(object sender, EventArgs e)
        {
            ...
            table.Columns.Add("样本编号", typeof(string));
            table.Columns.Add("位置", typeof(int));
            table.Columns.Add("样本类型", typeof(string));
            table.Columns.Add("项目名称", typeof(string));
            table.Columns.Add("PMT", typeof(int));
            table.Columns.Add("浓度", typeof(string));
            ...
            DataTable tbtbProject = DbHelperOleDb.Query(@"select ValueUnit,RangeType,ExpiryDate,ValueRange1,ValueRange2 from tbProject where ShortName = 'CEA'").Tables[0];
            string Range1 = "";
            string Range2 = "";
            int ExpiryDate = 0;
            int NumResult = 0;
            foreach (DataRow dr in tbtbProject.Rows)
            {
                if (dr != null)
                {
                    ...
                    NumResult = Convert.ToInt32(dr["RangeType"]);
                }
            }
            db = new DbHelperOleDb(0);
            //线性范围最小值
            double MinValue = double.Parse(DbHelperOleDb.GetSingle(@"select MinValue from tbProject where ShortName = 'CEA'").ToString());
            //线性范围最大值
            double MaxValue = double.Parse(DbHelperOleDb.GetSingle(@"select MaxValue from tbProject where ShortName = 'CEA'").ToString());
            int TestID = 0;
            for (int i = 0; i < dt.Rows.Count; i++)
            {

                DataRow _dtdr = dt.Rows[i];
                int RepeatCount = int.Parse(_dtdr["RepeatCount"].ToString());
                for (int j = 0; j < RepeatCount; j++)
                {
                    TestID++;
                    DataRow dr = table.NewRow();
                    dr["样本编号"] = _dtdr["SampleNo"];
                    dr["位置"] = _dtdr["Position"];
                    dr["样本类型"] = _dtdr["SampleType"];
                    dr["项目名称"] = "CEA";
                    dr["单位"] = "ng/mL";
                    dr["序号"] = TestID;
                    #region 是否使用实验预设功能 2019-02-26 zlx add
                    string samplenametemp = dr["样本类型"].ToString();
                    bool isSample = false;
                    if (samplenametemp == "血清" || samplenametemp == "尿液" || samplenametemp == "体液" || samplenametemp == "定标液" || samplenametemp.Contains("质控品"))
                    {
                        samplenametemp = dr["样本编号"].ToString();
                        isSample = true;
                    ...
                    string c = XmlOperation.FindTestNumber("CEA", samplenametemp);
                    //Convert.ToDouble(concentration).ToString("F" + NumResult + "");
                    if (c != "-1" /*&& c != "0"*/)
                    {
                        if (isSample)
                        {
                            //dr["浓度"] = c;
                            dr["浓度"] = Convert.ToDouble(c).ToString("F" + NumResult + "");
                            double tempnum = ExperimentNumberCalculateFactory.CalculateFactory("CEA", c, true);
                            dr["PMT"] = (int)(tempnum == -1 ? 0 : tempnum);
                            if (dr["样本编号"].ToString() == "Auto027" || dr["样本编号"].ToString() == "Auto028")
                            {
                                if (TestID % 2 == 0)
                                    dr["PMT"] = new Random().Next(7000, 8700);
                                else
                                    dr["PMT"] = new Random().Next(8700, 9500);
                            }
                            #region 重新计算标识
                            if (double.Parse(dr["浓度"].ToString()) < MinValue)
                            {
                                dr["浓度"] = MinValue.ToString();
                                dr["结果"] = "不在线性范围之内";
                            }
                            else if (double.Parse(dr["浓度"].ToString()) > MaxValue)
                            {
                                dr["浓度"] = MaxValue.ToString();
                                dr["结果"] = "不在线性范围之内";
                            }
                            else
                            {
                                if (Range1 != "")
                                {
                                    if (Range1.Contains("-"))
                                    {
                                        ...
                                        if (double.Parse(dr["浓度"].ToString()) < double.Parse(ranges[0]))
                                        {
                                            dr["结果"] = "↓";
                                        }
                                        else if (double.Parse(dr["浓度"].ToString()) > double.Parse(ranges[1]))
                                        {
                                            dr["结果"] = "↑";
                                        }
                                        else
                                    ...
                                    {
                                        if (double.Parse(dr["浓度"].ToString()) >= double.Parse(Range1.Substring(1)))
                                        {
                                            dr["结果"] = "↑";
                                        }
                                        else
                                    ...
                                    {
                                        if (double.Parse(dr["浓度"].ToString()) > double.Parse(Range1.Substring(2)))
                                        {
                                            dr["结果"] = "↑";
                                        }
                                        else
                        ...
                        {
                            if (samplenametemp == "标准品A")
                                dr["浓度"] = 0;
                            else if (samplenametemp == "标准品B")
                                dr["浓度"] = 5;
                            else if (samplenametemp == "标准品C")
                                dr["浓度"] = 10;
                            else if (samplenametemp == "标准品D")
                                dr["浓度"] = 25;
                            else if (samplenametemp == "标准品E")
                                dr["浓度"] = 50;
                            else if (samplenametemp == "标准品F")
                                dr["浓度"] = 150;
                            dr["PMT"] = (int)ExperimentNumberCalculateFactory.StringToDouble(c);
                        }
                    }
            ...
            dgvResultData.DataSource = table;
        }
    }
}
