...
namespace BioBaseCLIA.Run
{
    public partial class frmWorkList : frmParent
    {
        ...
        /// 反应盘待使用空白反应管个数
        /// </summary>
        int toUsedTube = 15;//modify y 20180522 10=>15
        /// <summary>
        /// 未开始进行实验的反应管的id
        /// </summary>
        int NoStartTestId = 1;
        ...
        void EmergencySampleSch()
        {
            ...
            if (RunFlag==(int)RunFlagStart.IsRuning)
            {
                ...
                if (EmergencyFlag)
                {
                    ...
                    DbHelperOleDb.ExecuteSql(@"update tbSampleInfo set Emergency = 3 where Status = 0 and Emergency = 2 and SendDateTime >=#"
                    ...
                    for (int i = 0; i < dtprosample.Rows.Count; i++)
                    {
                      ...
                      ty.SampleNo == dtprosample.Rows[i]["SampleNo"].ToString()).Count;
                        ...
                        {
                            DataTable dtproinfo = bllproject.GetList(" ShortName ='" + projectName[j] + "'").Tables[0];
                            dtSampleInfo.Rows.Add(dtprosample.Rows[i]["SampleID"], dtprosample.Rows[i]["SampleType"],
                                ...
                                dtprosample.Rows[i]["SampleContainer"], 4, dtproinfo.Rows[0]["ProjectProcedure"], projectName[j]);
                        }
                    }
                    #endregion
                ...
                else
                {
                    ...
                    DbHelperOleDb.ExecuteSql(@"update tbSampleInfo set Emergency = 1 where Status = 0 and Emergency = 0 and SendDateTime >=#"
                                      + DateTime.Now.ToString("yyyy-MM-dd") + "#and SendDateTime <#"
                                      + DateTime.Now.AddDays(1).ToString("yyyy-MM-dd") + "#");
                    db = new DbHelperOleDb(0);
                    for (int i = 0; i < dtprosample.Rows.Count; i++)
                ...
                for (int i = 0; i < dtprosample.Rows.Count; i++)
                {
                    ...
                    for (int j = 0; j < projectName.Length; j++)
                    {
                        ...
                        dtSampleInfo.Rows.Add(dtprosample.Rows[i]["SampleID"], dtprosample.Rows[i]["SampleType"],
                            ...
                            dtproinfo.Rows[0]["ProjectProcedure"], projectName[j]);
                    }
                }
                #endregion
                if (dtSampleInfo.Rows.Count == 0 || dtSampleInfo == null)
        ...
        public DataTable MethodInit(DataTable dtSampleInfo)
        {
            ...
            for (int i = 0; i < dtSampleInfo.Rows.Count; i++)
            {
                ...
                {
                    if (dtTestData.Rows.Count >= int.Parse(dtSampleInfo.Rows[i]["RepeatCount"].ToString()))
                        continue;
                    else
                        dtSampleInfo.Rows[i]["RepeatCount"] = int.Parse(dtSampleInfo.Rows[i]["RepeatCount"].
                            ToString()) - dtTestData.Rows.Count;
                }
                ...
                switch (lisTS[index].TestScheduleStep)
                {
                    case TestSchedule.ExperimentScheduleStep.AddLiquidTube:
                        for (int i = 0; i < freeTime.Count; i++)
                        {
                            ...
                            int maxTime = int.Parse(minMaxTime[1]);
                            if (
                                addLiquidStartTime >= minTime && addLiquidStartTime + lisTS[index].TimeLengh <= maxTime)
                            {
                                isAllow = true;
                                break;
                            }
        ...
        bool MachineInit()
        {
            ...
            //if (!CleanTrayWashPipeline()) return false;
            //if (RunFlag != (int)RunFlagStart.IsRuning) return false;
            reactTrayTubeClear();
            if (RunFlag != (int)RunFlagStart.IsRuning) return false;
        ...
        bool RunConditionJudge()
        {
            ...
            foreach (var item in ItemNames)
            {
               List<ReagentIniInfo> itemRiInfo = lisRIinfo.FindAll(ty => (ty.ItemName == item.Key));
                ...
                if (list.Count > 0)
                {
                    ...
                    string DiluteCount = DbHelperOleDb.GetSingle(@"select DiluteCount from tbProject where ShortName ='" + item.Key + "'").ToString();
                    ...
                    foreach (ReagentIniInfo RiInfo in itemRiInfo)
                    {
                        ...
                        if (ReagentType == "0")
                            continue;
                        if (RiInfo.LeftReagent1 > DiuNoUsePro)
                            DiuVolLeftCount = DiuVolLeftCount + RiInfo.LeftReagent1 - DiuNoUsePro;
                        if (RiInfo.LeftReagent2 > DiuNoUsePro)
                            DiuVolLeftCount = DiuVolLeftCount + RiInfo.LeftReagent2 - DiuNoUsePro;
                        if (RiInfo.LeftReagent3 > DiuNoUsePro)
                            DiuVolLeftCount = DiuVolLeftCount + RiInfo.LeftReagent3 - DiuNoUsePro;
                        /*
                    ...
                    foreach (TestSchedule AddTest in list)
                    {
                        ...
                        if (int.Parse(DiluteCount) > 1)
                        {
                            ...
                            GetSampleV = int.Parse(diuList[diuList.Count - 1].Split(';')[0]);
                            //foreach (string duitec in DiluteName.Split(';'))
                            //{
                            //    if(duitec!="")
                            //        SumDiluteVol = SumDiluteVol + getDiuVol(_diuSumVol, int.Parse(duitec))[1]+abanDiuPro;
                            //}
                        }
                        ...
                        if (ExtraDiuCount > 1)
                        {
                            ...
                            //foreach (string duitec in diuInfo.Split(';'))
                            //{
                            //    if (duitec != "")
                            //        SumDiluteVol = SumDiluteVol + getDiuVol(_diuSumVol, int.Parse(duitec))[1] + abanDiuPro;
                            //}
                ...
                */
                ...
                DataTable dtItemInfo = DbHelperOleDb.Query(@"select ProjectType,CalPointConc,CalPointNumber,ExpiryDate from tbProject where ShortName ='" + item.Key + "'").Tables[0];
                ...
                if (regenBatchNums.Count > 0)
                {
                    foreach (var reBNum in regenBatchNums)
                    {
                        ...
                        DataTable tbScalingResult = DbHelperOleDb.Query(@"select Points,ActiveDate from tbScalingResult where ItemName ='"
                                ...
                                if (int.Parse(dtScal.Rows[2][1].ToString()) > 0 && int.Parse(dtScal.Rows[4][1].ToString()) > 0)//C.E两点数量不为0
                                {
                                    ...
                                    if (ExitsMainCurve) //有主曲线
                                    {
                                        ...
                                        if (lisSameItem.FindAll(ty => (ty.SampleType.Contains("标准品"))).Count == 0)
                                        {
                                            ...
                                            Calculater er = GetCalculater(DataMain, tempdt);
                                            if (er.R2 < 0.99)
                                            {
            ...
            foreach (TestItem item in QCList)
            {
                ...
                DataTable dtQCInfo = DbHelperOleDb.Query(@"select QCID,Batch,QCLevel from tbQC where status = '1' and ProjectName = '"
        ...
        /// 获取定标公式信息
        /// </summary>
        private Calculater GetCalculater(DataTable dtMain, DataTable dtCaculate)
        {
            //新建定标方程类的实例
            CalculateFactory ft = new CalculateFactory();
            List<Data_Value> CurveData = new List<Data_Value>();
            Calculater er = ft.getCaler(0);
           ...
           if(!AddTubeInCleanTray())
            ...
            if (RunFlag == (int)RunFlagStart.IsRuning)
            {
              ...
              if(!AddTubeInCleanTray())
                ...
                for (int i = 0; i < 3; i++)
                {
                    if (RunFlag != (int)RunFlagStart.IsRuning || NetCom3.Instance.stopsendFlag ) break;
                    CleanTrayWash(1);
                    CleanTrayMovePace(-1);
                    CleanTrayWash(2);
                    CleanTrayMovePace(-1);
                ...
                if (lisTestSchedule.Count > 0)
                {
                    ...
                    waitTime:
                        ...
                        if(AddTubeStop.Count > 0 && !TubeStop)
                        {
                            ...
                            for (int i = 0; i < AddTubeStopCopy.Count;i++ )
                            {
                                ...
                                /*
                                 List<int> SAddTube = new List<int>();
                                 List<int> CAddTube;
                                 lock (AddTubeStop)
                                 {
                                     CAddTube = AddTubeStop.GetRange(0, AddTubeStop.Count);
                                     AddTubeStop.Clear();
                                 }
                                 foreach (int AddTube in CAddTube)
                                 {
                                     if (!TubeStop)
                                     {
                                         int OnePos = int.Parse(OperateIniFile.ReadIniData("Tube", "TubePos", "1", iniPathSubstrateTube));
                                         rackToReact(AddTube);
                                         if (int.Parse(OperateIniFile.ReadIniData("ReactTrayInfo", "no" + AddTube, "", iniPathReactTrayInfo)) == 1)
                                             SAddTube.Add(AddTube);
                                     }
                                 }
                                 if (SAddTube.Count == CAddTube.Count)
                                 {
                                     SAddTube.Clear();
                                     CAddTube.Clear();
                                 }
                                 ...
                                 {
                                     foreach (int AddTube in SAddTube)
                                     {
                                         CAddTube.Remove(AddTube);
                                     }
                                 }
                                 if (CAddTube.Count > 0)
                                 {
                                     foreach (int AddTube in CAddTube)
                                     {
                                         if (!AddTubeStop.Contains(AddTube))
                                         {
                                             lock (AddTubeStop)
                                             {
                                                 AddTubeStop.Add(AddTube);
                                             }
                                         }
                                     }
                                 }
                        ...
                        switch (TestStep.TestScheduleStep)
                        {
                            ...
                            case TestSchedule.ExperimentScheduleStep.AddLiquidTube:
                               ...
                               #region 稀释液试剂是否够用判断
                                ...
                                {
                                    DbHelperOleDb db = new DbHelperOleDb(0);
                                    string DiluteCount = DbHelperOleDb.GetSingle(@"select DiluteCount from tbProject where ShortName ='" + TestStep.ItemName + "'").ToString();
                                    db = new DbHelperOleDb(0);
                                    string DiluteName = DbHelperOleDb.GetSingle(@"select DiluteName from tbProject where ShortName ='" + TestStep.ItemName + "'").ToString();
                                ...
                                {
                                    if (rowRg["ReagentType"].ToString() == "1" && IsDiu)
                                    {
                                        if ((Convert.ToInt32(rowRg["leftoverTestR1"].ToString()) - DiuNoUsePro) >DiuNum)
                                            IsDiu = false;
                                        if ((Convert.ToInt32(rowRg["leftoverTestR2"].ToString()) - DiuNoUsePro) > DiuNum)
                                            IsDiu = false;
                                        if ((Convert.ToInt32(rowRg["leftoverTestR3"].ToString()) - DiuNoUsePro) > DiuNum)
                                            IsDiu = false;
                                    }
                                    else
                                ...
                                if ((int.Parse(TestStep.dilutionTimes)>1&&IsDiu) || Rgcounum <= 0)
                                {
                                    ...
                                    GetNoStartList(TestStep.ItemName);
                                    setmainformbutten();
                                    break;
                                }
                                #endregion
                               #region 底物是否够用判断
                                ...
                                if (substrateNum1 + substrateNum2 <= Sampleing)
                                {
                                    SubstrateStop= true;
                    ...
                    while (RunFlag==(int)RunFlagStart.IsRuning)
                    {
                        ...
                        if (((SampleNumCurrent<=0 && BTestResult.Count != 0) || (SampleNumCurrent == StopList.Count && StopList.Count > 0)) && MoveTubeUseFlag == false && !washTrayTube() && !ReactTrayTube())//2018-10-09 zlx mod
                        {
                            ...
                            {
                                if(frmMain.StopFlag[0] || frmMain.StopFlag[1] || frmMain.StopFlag[2])
                                    MessageBox.Show("仪器缺液，部分实验已完成！详细信息请在日志查看！", "实验状态", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1, MessageBoxOptions.ServiceNotification);
                                else
                                    MessageBox.Show("供应品缺少，部分实验已完成！详细信息请在日志查看！", "实验状态", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1, MessageBoxOptions.ServiceNotification);
                            }
                            else
        ...
        public void GetNoStartList(string projectName)
        {
        ...
        public DataTable NoStartList(int testid)
        {
            DataTable dt = new DataTable();
            // 列强制转换
            for (int count = 0; count < dgvWorkListData.Columns.Count; count++)
            {
                DataColumn dc = new DataColumn(dgvWorkListData.Columns[count].Name.ToString());
                dt.Columns.Add(dc);
            }
            // 循环行
            for (int count = 0; count < dgvWorkListData.Rows.Count; count++)
            {
                if (int.Parse(dgvWorkListData.Rows[count].Cells[1].Value.ToString())>= NoStartTestId)
                    continue;
                DataRow dr = dt.NewRow();
                for (int countsub = 0; countsub < dgvWorkListData.Columns.Count; countsub++)
                {
                    dr[countsub] = Convert.ToString(dgvWorkListData.Rows[count].Cells[countsub].Value);
                }
                dt.Rows.Add(dr);
            }
            return dt;
        ...
        private void addLiquid(object testS)
        {
            ...
            //{
                ...
                //获取试剂盘上对应实验名称的试剂信息
                DataRow[] drRg = dtRgInfo.Select("RgName='" + testTempS.ItemName.ToString() + "'");
                int rgPos = 0;
                int rgindex = -1;
                ...
                switch (testTempS.TestScheduleStep)
                {
                    case TestSchedule.ExperimentScheduleStep.AddLiquidTube:
                        ...
                        DataTable dtReactTrayInfo;//温育盘信息表  20180530 zlx add
                        for (int j = 0; j < TestStepSingle.Length; j++)
                        {
                            ...
                            if (TestStepSingle[j] == "D")
                            {
                                ...
                                string DiluteCount = DbHelperOleDb.GetSingle(@"select DiluteCount from tbProject where ShortName 
                                                                             = '" + testTempS.ItemName + "'").ToString();
                                string DiluteName = DbHelperOleDb.GetSingle(@"select DiluteName from tbProject where ShortName ='" + testTempS.ItemName+ "'").ToString();
                                ...
                                if (DiluteName != "1")
                                {
                                    ...
                                    string Pos = "";
                                    string LeftReagent = "0";
                                    for (int i=0;i < diuList.Count;i++)
                                    {
                                        int SampleVol = int.Parse(diuList[i].Split(';')[0]);
                                        int DiuVol = int.Parse(diuList[i].Split(';')[1]);
                                        #region 获取稀释液位置
                                        for (int jj = 0; jj < drRg.Length; jj++)
                                        {
                                            if (drRg[jj]["ReagentType"].ToString() != "1") continue;
                                            int SumdiuVol = int.Parse(OperateIniFile.ReadIniData("ReagentPos" + drRg[jj]["Postion"].ToString(), "leftDiuVol", "0", iniPathReagentTrayInfo));
                                            if (SumdiuVol < DiuVol) continue;
                                            string LeftReagent1 = drRg[jj]["leftoverTestR1"].ToString();
                                            if (LeftReagent1 != "" && int.Parse(LeftReagent1) > DiuVol + abanDiuPro + DiuNoUsePro)
                                            {
                                                rgPos = int.Parse(drRg[jj]["Postion"].ToString());
                                                Pos = "R1";
                                                LeftReagent = LeftReagent1;
                                            }
                                            string LeftReagent2 = drRg[jj]["leftoverTestR2"].ToString();
                                            if (LeftReagent2 != "" && int.Parse(LeftReagent2) > DiuVol + abanDiuPro + DiuNoUsePro && int.Parse(LeftReagent2) < int.Parse(LeftReagent))
                                            {
                                                rgPos = int.Parse(drRg[jj]["Postion"].ToString());
                                                Pos = "R2";
                                                LeftReagent = LeftReagent2;
                                            }
                                            string LeftReagent3 = drRg[jj]["leftoverTestR3"].ToString();
                                            if (LeftReagent3 != "" && int.Parse(LeftReagent3) > DiuVol + abanDiuPro + DiuNoUsePro && int.Parse(LeftReagent3) < int.Parse(LeftReagent))
                                            {
                                                rgPos = int.Parse(drRg[jj]["Postion"].ToString());
                                                Pos = "R3";
                                                LeftReagent = LeftReagent3;
                                            }
                                            if (rgPos > 0 && Pos != "" && LeftReagent != "") break;
                                        }
                                        #endregion
                                        if (rgPos > 0)
                                        {
                                            ...
                                            #region 加稀释液
                                            AddErrorCount = 0;
                                            AddErrorCount = AddLiquid(rgPos, pos, Pos, DiuVol, int.Parse(LeftReagent));
                                            ...
                                            Again:
                                                ...
                                                if (SendAgain(againSend, 0) == (int)ErrorState.Sendfailure)
                                                    goto Again;
                                                else if (SendAgain(againSend, 0) == (int)ErrorState.IsKnocked)
                                                    AddErrorCount++;
                                                #endregion
                                            }
                                            //配置文件进行修改
                                            ...
                                            if (AddErrorCount > 0)
                                            {
                                                ...
                                                {
                                                    NetCom3.Instance.stopsendFlag = true;
                                                    ShowWarnInfo("加样针加样本时加样本时发生撞针", "加样", 1);
                                                    AllStop();
                                                }
                                                else
                                                {
                            ...
                            else if (TestStepSingle[j] == "R1")
                            {
                                ...
                                if (drRg.Length > 0)
                                {
                                    ...
                                    {
                                        if (drRg[g]["ReagentType"].ToString() == "1") continue;
                                        ...
                                        if (dgvWorkListData != null && dgvWorkListData.Rows.Count != 0)
                                            {
                                                this.BeginInvoke(new Action(() =>
                                                {
                                                    dgvWorkListData.Rows[testTempS.TestID - 1].Cells["RegentBatch"].Value = Batch;
                    ...
                    case TestSchedule.ExperimentScheduleStep.AddBeads:
                        ...
                        if (rgPos > 0)
                        {
                            ...
                            //drRg[rgindex]["leftoverTestR4"] = leftR4 - 1;
                            for (int i = 0; i < drRg.Length; i++)
                            {
                                if (int.Parse(drRg[i]["Postion"].ToString()) == rgPos)
        ...
        /// <param name="step">加样样步骤</param>
        /// <param name="leftdiuVol">加样体积</param>
        /// <param name="ReagentType">加样类型</param>
        private int AddLiquid(int rgPos, int pos, string step, int FirstDiu, int leftdiuVol)
        {
            int AddErrorCount = 0;
            string LeftReagent = "";
            string Order = "";
            string leftoverTestR = "";
            if(step == "R1")
            {
                Order = "EB 90 31 02 03 ";
                LeftReagent = "LeftReagent1";
                leftoverTestR = "leftoverTestR1";
            }
            if (step == "R2")
            {
                Order = "EB 90 31 02 04 ";
                LeftReagent = "LeftReagent2";
                leftoverTestR = "leftoverTestR2";
            }
            if (step == "R3")
            {
                Order = "EB 90 31 02 05 ";
                LeftReagent = "LeftReagent3";
                leftoverTestR = "leftoverTestR3";
            }
            string LeftdiuVol = leftdiuVol.ToString("x2");
            while (LeftdiuVol.Length < 4)
            {
                LeftdiuVol = "0" + LeftdiuVol;
            }
            //string leftdiuVol = OperateIniFile.ReadIniData("ReagentPos" + rgPos.ToString(), LeftReagent, "0", iniPathReagentTrayInfo);
            NetCom3.Instance.Send(NetCom3.Cover( Order + rgPos.ToString("x2") + " " + pos.ToString("x2")
                                          + " " + FirstDiu.ToString("x2") + " " + LeftdiuVol.ToString().Substring(0, 2) + " " + LeftdiuVol.ToString().Substring(2, 2)), 0);
            //指令未执行完成进行等待
            ...
            {
                if (int.Parse(drRg[i]["Postion"].ToString()) == rgPos)
                    drRg[i][leftoverTestR] = leftdiuVol - FirstDiu - abanDiuPro;
            }
            //diuleftVol[rgPos - 1] = (int.Parse(diuleftVol[rgPos - 1]) - (int)(FirstDiu + abanDiuPro)).ToString();
            OperateIniFile.WriteIniData("ReagentPos" + rgPos.ToString(), LeftReagent, (leftdiuVol - FirstDiu - abanDiuPro).ToString(), iniPathReagentTrayInfo);
            string leftDiuVol = OperateIniFile.ReadIniData("ReagentPos" + rgPos.ToString(), "leftDiuVol", "0", iniPathReagentTrayInfo);
            OperateIniFile.WriteIniData("ReagentPos" + rgPos.ToString(), "leftDiuVol", (int.Parse(leftDiuVol) - FirstDiu- abanDiuPro).ToString(), iniPathReagentTrayInfo);
            #endregion
            ...
            if (!NetCom3.Instance.MoveQuery())
            {
                ...
                if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsNull)
                {
                   ...
                   else
                   {
                       ...
                       dbtnRackStatus();
                       setmainformbutten();
                       LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "理杯机抓管多次抓空!实验停止进行加样！");
                   }
                ...
                else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.LackTube )
                {
                    ...
                    dbtnRackStatus();
                    setmainformbutten();
                    LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "理杯机无管!实验停止进行加样！");
                }
                else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.Sendfailure)
                {
                    if (NetCom3.Instance.waitAndAgainSend != null && NetCom3.Instance.waitAndAgainSend is Thread)
                ...
                else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsKnocked)
                {
                    ...
                    else
                    {
                        setmainformbutten();
                        NetCom3.Instance.stopsendFlag = true;
                        LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "移管手向温育盘抓新管时发生撞管！");
                    ...
                    if (!NetCom3.Instance.MoveQuery())
                    {
                        ...
                        if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsKnocked)
                        {
                            ...
                            else
                            {
                                setmainformbutten();
                                NetCom3.Instance.stopsendFlag = true;
                                LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "温育盘！");
                ...
                else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.OverTime)
                {
                    ...
                    LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "移管手在管架向温育盘抓管时接收数据超时！");
                    AllStop();
                    DialogResult tempresult = MessageBox.Show("移管手在管架向温育盘抓管时接收数据超时，实验将进行停止！", "移管手错误", MessageBoxButtons.OKCancel, MessageBoxIcon.Error);
                }
                #endregion
            }
            else
        ...
        private void timerWash_Tick(object sender, EventArgs e)
        {
            ...
            while (WashTrayUseFlag || WashTurnFlag || NetCom3.Instance.stopsendFlag || ClamptubeWash())
            {
                Thread.Sleep(100);
            }
        ...
        private void  RemoveTestList(TestSchedule testSchedule)//add y 20180727  整理testschould列表，将进度中撞针的实验设置为跳过，以实现可以继续剩下的实验
        {
            ...
            if(TubeStop)
                ...
                thread = new Thread(new ThreadStart(() => { MessageBox.Show("编号为:" + TestnameTemp + " 的实验因为发生撞针，无法继续，已经排除，请在稍后重新运行此实验", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information); }));
            if (thread!=null)
            {
                        ...
                        if (putpos[0] == "1")
                        {
                            ...
                            if (!NetCom3.Instance.MoveQuery())
                            {
                                ...
                                else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.LackTube)
                                {
                                    ...
                                    dbtnRackStatus();
                                    setmainformbutten();
                                    LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "理杯机缺管!实验停止进行加样！");
                                }
                                else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.Sendfailure)
                                {
                                    if (NetCom3.Instance.waitAndAgainSend != null && NetCom3.Instance.waitAndAgainSend is Thread)
                                ...
                                else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsKnocked)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        NetCom3.Instance.stopsendFlag = true;
                                        LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "移管手在管架向温育盘抓管时发生撞管！");
                                        DialogResult tempresult = MessageBox.Show("移管手在管架向温育盘抓管时发生撞管，实验将进行停止！", "移管手错误", MessageBoxButtons.OKCancel, MessageBoxIcon.Error);
                                        AllStop();
                                    }
                                }
                                else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.putKnocked)
                            ...
                            ClearMove:
                                ...
                                if(!NetCom3.Instance.MoveQuery()&& NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsKnocked)
                                {
                                    ...
                                    else
                                    {
                                        setmainformbutten();
                                        NetCom3.Instance.stopsendFlag = true;
                                        LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "移管手在温育盘扔废管时发生撞管！撞管位置为：" + tubeHoleNum);
                        ...
                        AgainNewMove:
                            ...
                            if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsNull)
                            {
                                ...
                                else
                                {
                                    setmainformbutten();
                                    NetCom3.Instance.stopsendFlag = true;
                                    LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "移管手在清洗盘扔废管时时多次抓空！抓空位置为：" + tubeHoleNum);
                            ...
                            else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsKnocked)
                            {
                                ...
                                else
                                {
                                    setmainformbutten();
                                    NetCom3.Instance.stopsendFlag = true;
                                    LogFileAlarm.Instance.Write(DateTime.Now.ToString("HH-mm-ss") + " *** " + "错误" + " *** " + "未读" + " *** " + "移管手在清洗盘扔废管时时发生撞管！撞管位置为：" + tubeHoleNum);
                    ...
                    AgainNewMove:
                        ...
                        else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.IsKnocked)
                        {
                            ...
                            else
                            {
                                ...
                                DialogResult tempresult = MessageBox.Show("移管手在反应盘扔废管时发生撞管，实验将进行停止！", "移管手错误", MessageBoxButtons.OKCancel, MessageBoxIcon.Error);
                                AllStop();
                            }
                        ...
                        else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.OverTime)
                        {
                            ...
                            DialogResult tempresult = MessageBox.Show("移管手在反应盘扔废管时接收数据超时，实验将进行停止！", "移管手错误", MessageBoxButtons.OKCancel, MessageBoxIcon.Error);
                            AllStop();
                            break;
                        }
                        #endregion
                        ...
                        if (!NetCom3.Instance.MoveQuery())
                        {
                           ...
                           #region 发生异常处理
                            ...
                            else if (NetCom3.Instance.MoverrorFlag == (int)ErrorState.OverTime)
                            {
                                ...
                                DialogResult tempresult = MessageBox.Show("移管手在反应盘夹管到清洗盘时接收数据超时，实验将进行停止！", "移管手错误", MessageBoxButtons.OKCancel, MessageBoxIcon.Error);
                                AllStop();
                                break;
                            }
                        #endregion
        ...
        void CalculatResult(int testid, int pmt)
        {
            ...
            {
                DbHelperOleDb.ExecuteSql(@"update tbSampleInfo set Status = 1 where SampleID = " + int.Parse
            ...
            //查询当前项目范围
            //2018-08-03 zlx mod
            DataTable tbtbProject = DbHelperOleDb.Query(@"select ValueUnit,RangeType,ExpiryDate,VRangeType,ValueRange1,ValueRange2 from tbProject where ShortName = '" + ItemName + "'").Tables[0];
            string Range1 = "";
            string Range2 = "";
            double MaxValue = 0;
            double MinValue = 0;
            ...
            if (CurrentScal.testType == 0)//定性实验
            {
                ...
                if (CurrentScal.Num == "0")//实验中无正在做的标准品或定标液，使用历史定标
                {
                    db = new DbHelperOleDb(1);
                    //2018-08-03 zlx mod
                    DataTable tbScalingResult = DbHelperOleDb.Query(@"select Points,ActiveDate from tbScalingResult where ItemName = '" + ItemName
            ...
            else
            {
                ...
                if (sampleType.Contains("标准品"))
                {
                    ...
                    else
                    {
                        ...
                        if (rows.Length == ScalSumNum)//两点定标只有两点
                        {
                            ...
                            db = new DbHelperOleDb(1);
                            string points = DbHelperOleDb.GetSingle(@"select Points from tbMainScalCurve where ItemName = '"
                ...
                else
                {
                    ...
                    MinValue = double.Parse(DbHelperOleDb.GetSingle(@"select MinValue from tbProject where ShortName = '" + ItemName + "'").ToString());
                    //线性范围最大值 2018-10-20 zlx mod
                    db = new DbHelperOleDb(0);//2018-09-04 zlx add
                    MaxValue = double.Parse(DbHelperOleDb.GetSingle(@"select MaxValue from tbProject where ShortName = '" + ItemName + "'").ToString());
                    //正则表达式 表示汉字范围;
                    ...
                    if (dtSampleRunInfo.Rows.Count > 0)
                    {
                        db = new DbHelperOleDb(0);
                        //项目中的稀释倍数
                        int DiuTimes = int.Parse(DbHelperOleDb.GetSingle(@"select DiluteCount from tbProject where ShortName = '" + ItemName + "'").ToString());
                        //该样本编号
            ...
            #region 是否使用实验预设功能 2019-02-26 zlx add
            string PredictSet=OperateIniFile.ReadInIPara("TestResultSet", "PredictSet");
            if (PredictSet == "1")
            {
               
                string samplenametemp = sampleType;
                bool isSample = false;
                if (sampleType == "血清" || sampleType == "尿液" || sampleType == "体液" || sampleType == "定标液")
                {
                    samplenametemp = testResult.SampleNo;
                    isSample = true;
                    string PredictNum = OperateIniFile.ReadInIPara("TestResultSet", "PredictNum");
                    if (PredictNum != "" || int.Parse(PredictNum) > 0)
                    {
                        samplenametemp = XmlOperation.PredictStart + (int.Parse(samplenametemp.Substring(samplenametemp.Length - 3, 3)) % int.Parse(PredictNum)).ToString("000");
                    }
                    else
                    {
                        samplenametemp = XmlOperation.PredictStart + int.Parse(samplenametemp.Substring(samplenametemp.Length - 3, 3)).ToString("000");
                    }
                }
               
                XmlOperation.GetXmlFile();
                string c = XmlOperation.FindTestNumber(ItemName, samplenametemp);
                if (c != "-1" /*&& c != "0"*/)
                {
                    if (isSample)
                    {
                        testResult.concentration = c;
                        double tempnum = ExperimentNumberCalculateFactory.CalculateFactory(ItemName, c, true);
                        testResult.PMT = (int)(tempnum == -1 ? 0 : tempnum);
                        #region 重新计算标识
                        if (double.Parse(testResult.concentration) < MinValue)
                        {
                            //testResult.concentration = MinValue.ToString();
                            result = "不在线性范围之内";
                        }
                        else if (double.Parse(testResult.concentration) > MaxValue)
                        {
                            //testResult.concentration = MaxValue.ToString();
                            result = "不在线性范围之内";
                        }
                        else
                        {
                            if(Range1 != "")
                            {
                                ...
                                {
                                    string[] ranges = Range1.Split('-');
                                    if (double.Parse(testResult.concentration) < double.Parse(ranges[0]))
                                    {
                                        testResult.Result = "↓";
                                    }
                                    else if (double.Parse(testResult.concentration) > double.Parse(ranges[1]))
                                    {
                                        testResult.Result = "↑";
                                    }
                                    else
                                        testResult.Result = "正常";
                                }
                                else if (Range1.Contains("<"))
                                {
                                    if (double.Parse(testResult.concentration) >= double.Parse(Range1.Substring(1)))
                                    {
                                        testResult.Result = "↑";
                                    }
                                    else
                                ...
                                {
                                    if (double.Parse(testResult.concentration) > double.Parse(Range1.Substring(2)))
                                    {
                                        testResult.Result = "↑";
                                    }
                                    else
                    ...
                    {
                        testResult.PMT = (int)ExperimentNumberCalculateFactory.StringToDouble(c);
                        //double tempnum = ExperimentNumberCalculateFactory.CalculateFactory(ItemName, c, false);
                        //testResult.concentration = (tempnum == -1 ? 0 : tempnum).ToString("###.###");
                    }

                }
                XmlOperation.EndXmlFile();
            }
            #endregion
        ...
        void Calculate(object obCaculate)
        {
            ...
            lock (locker)
            {
                ...
                if (dtCaculate.Rows[0]["ItemType"].ToString() == "0")
                {
                    db = new DbHelperOleDb(0);
                    //计算参数
                    double calulatePara = double.Parse(DbHelperOleDb.GetSingle(@"select CalculateMethod from tbProject where ShortName = '"
        ...
        void Goon()
        {
            ...
            NetCom3.ComWait.Reset();
            //BStopFlag = true;//2018-10-08 zlx mod
            #region 获取已经开始实验的样本的空闲时间
            //获取已经开始运行的样本的进度表
            //List<TestSchedule> RunSchedule = lisTestSchedule.FindAll(tx => tx.TestID < NoStartTestId);
            //List<string> runFreeTime = new List<string>() { (sumTime + 2).ToString() + "-100000" };
            ...
            //            ts.TestScheduleStep == TestSchedule.ExperimentScheduleStep.AddSingleR)
            //        {
            //            if (ts.StartTime == minTime && ts.EndTime < maxTime)
            //            {
            //                runFreeTime[j] = (ts.EndTime + 1).ToString() + "-" + maxTime;
            //                break;
            //            }
            //            else if (ts.StartTime > minTime && ts.EndTime < maxTime)
            //            {
            //                int index = 0;
            //                runFreeTime[j] = (minTime).ToString() + "-" + (ts.StartTime - 1);
            //                index = j + 1;
            ...
            if (AllowDel())
            {
                ...
                for (int m = 0; m < dgvWorkListData.SelectedRows.Count; m++)
                {
                    ...
                    else
                    {
                        ...
                        int bDelete = DbHelperOleDb.ExecuteSql(@"delete from tbSampleInfo where Status =0 AND SampleNo='" + SampleNo + "'");
                        if (bDelete > 0)
                        {
